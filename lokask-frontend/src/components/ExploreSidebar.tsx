...
import { MapPin, Search, DollarSign, Globe, Loader2 } from "lucide-react";
...
export const ExploreSidebar = ({
  ...
  const { data: dbNiches, isLoading: isLoadingNiches } = useQuery({
    queryKey: ["niches"],
    queryFn: getNiches,
    staleTime: 1000 * 60 * 60, // Cache for 1 hour since niches rarely change
  });

  const { data: dbLanguages, isLoading: isLoadingLanguages } = useQuery({
    queryKey: ["languages"],
    ...
    <div className="w-[320px] bg-white rounded-[14px] border border-[#E5E7EB] p-6 flex flex-col gap-12 font-body sticky top-[121px]">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          {ratings.map((r) => (
            ...
            >
              <CustomCheckbox
                active={localFilters.minRating === r}
                onClick={() => {}}
              />
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2 pr-1">
          {isLoadingLanguages ? (
            <div className="flex items-center gap-2 text-[#717182] text-sm">
              <Loader2 className="w-4 h-4 animate-spin" /> Loading languages...
            </div>
          ) : (
            dbLanguages?.map((l) => (
              <div
              ...
              >
                <CustomCheckbox
                  active={localFilters.languages.includes(l)}
                  onClick={() => {}}
                />
                <span className="text-[#4A5565] text-[18px] font-normal leading-[28px]">
                  {l}
